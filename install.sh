...
# @@File             :  install.sh
# @@Description      :  Container installer script for youtubedl-web
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/youtubedl-web"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="youtubedl-web"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/youtubedl-web"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/youtubedl-web"
...
HUB_IMAGE_URL="casjaysdevdocker/youtubedl-web"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [youtubedl-web.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
if [ -n "$YOUTUBEDL_WEB_USERNAME" ]; then
  CONTAINER_USER_NAME="$YOUTUBEDL_WEB_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${YOUTUBEDL_WEB_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$YOUTUBEDL_WEB_PASSWORD" ]; then
  CONTAINER_USER_PASS="$YOUTUBEDL_WEB_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${YOUTUBEDL_WEB_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
